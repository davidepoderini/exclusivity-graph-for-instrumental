...
principle that makes scientific investigation possible.
...
A causal model is tipically represented by a directed acyclic graph (DAG), where
...
Indeed Bell experiment can be modelled using a causal DAG, as shown in
...
It can be easily proved (see Appendix) that no other odd anticycle beside
...
\begin{align}
    \Delta &= \min \{I_y : y \, \text{is a maximal stable labelling}\} \quad \text{where}
...
\appendix
\section{There are no cycles $C_7$ for $n \ge 7$ in the $d22$ instrumental scenario.}
